...
Markdown ファイルの先頭に YAML front matter を記載することで、JXls テンプレート内の変数名を明示的に指定できます。
...
| キー | 説明 |
|---|---|
| `var` | JXls テンプレート内で使用する変数名。未指定の場合はファイル名（拡張子なし）が使用される。 |
...
- front matter の `var` キーで JXls テンプレート内の変数名を指定できます（推奨）。
  `var` が未指定の場合はファイル名（拡張子なし）が変数名として使用されます。
