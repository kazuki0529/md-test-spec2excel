...
| キー | 説明 |
|---|---|
| `spec.var` | JXls テンプレート内で使用する変数名（予約キー）。未指定の場合はファイル名（拡張子なし）が使用される。 |
| その他のキー | ユーザ定義変数。テンプレートから `${varName.vars.キー名}` で参照できる。 |

> **例:** `spec.var: loginSpec` と指定すると、テンプレート内で `${loginSpec.title}` のように参照できます。  
> `feature: ログイン` を定義した場合は `${loginSpec.vars.feature}` で参照できます。

### `spec.var` に指定できる文字

`spec.var` の値は **Java / JEXL の識別子**として有効な文字列を指定してください。
...
- front matter の `spec.var` キーで JXls テンプレート内の変数名を指定できます（推奨）。
  `spec.var` が未指定の場合はファイル名（拡張子なし）が変数名として使用されます。
- front matter に `spec.var` 以外のキーを記載するとユーザ定義変数として扱われ、テンプレートから `${varName.vars.キー名}` で参照できます。
