...
| キー | 説明 |
|---|---|
| `spec.var` | JXls テンプレート内で使用する変数名。未指定の場合はファイル名（拡張子なし）が使用される。 |
| `vars` | テンプレートから `${varName.vars.キー名}` で参照できるユーザ定義変数のマップ。 |

> **例:** `spec.var: loginSpec` と指定すると、テンプレート内で `${loginSpec.title}` のように参照できます。  
> `vars.feature` に値を設定した場合は `${loginSpec.vars.feature}` で参照できます。
